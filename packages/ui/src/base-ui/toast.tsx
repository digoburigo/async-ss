...
import { buttonVariants } from "@acme/ui/base-ui/button";
      ...
      >
        {toasts.map((toast) => {
          ...
          return (
            <Toast.Root
              className={cn(
                "bg-popover text-popover-foreground absolute z-[calc(9999-var(--toast-index))] h-(--toast-calc-height) w-full rounded-md border bg-clip-padding px-3.5 py-3 shadow-lg select-none [transition:transform_.5s_cubic-bezier(.22,1,.36,1),opacity_.5s,height_.15s] before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] dark:bg-clip-border dark:before:shadow-[0_-1px_--theme(--color-white/8%)]",
