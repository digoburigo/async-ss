  ...
  useFrame((state) => {
    if (particlesRef.current) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        positions[i * 3 + 1] += Math.cos(state.clock.elapsedTime + i * 0.5) * 0.01
  ...
  return (
    <Float speed={1.8} rotationIntensity={0.4} floatIntensity={0.6}>
      <group ref={groupRef} position={[2, 0, 0]}>
        {/* Book/document stack */}
        {[0, 1, 2].map((i) => (
          <mesh key={i} position={[0, i * 0.3 - 0.3, 0]} rotation={[0, i * 0.2, 0]}>
  ...
  return (
    <div className="w-full h-[600px] bg-gradient-to-br from-blue-950 via-blue-900 to-blue-800 rounded-2xl overflow-hidden shadow-2xl">
      <Canvas camera={{ position: [0, 0, 12], fov: 50 }}>
        ...
        <spotLight position={[0, 10, 0]} angle={0.3} penumbra={1} intensity={1} color="#93c5fd" />
