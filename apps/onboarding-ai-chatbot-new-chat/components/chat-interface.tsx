...
function MessageSkeleton({ isUser = false }: { isUser?: boolean }) {
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <Card className={`max-w-[85%] md:max-w-[80%] p-3 md:p-4 ${isUser ? "bg-primary/50" : "bg-card"}`}>
        <div className="space-y-2">
          <Skeleton className={`h-4 w-48 ${isUser ? "bg-primary-foreground/20" : ""}`} />
          <Skeleton className={`h-4 w-32 ${isUser ? "bg-primary-foreground/20" : ""}`} />
        </div>
      </Card>
    </div>
  )
...
export function ChatInterface({ role, chatSessionId, title, onTitleChange, onBack }: ChatInterfaceProps) {
  ...
  const saveMessage = useCallback(
    async (messageRole: "user" | "assistant", content: string): Promise<string | null> => {
      ...
      try {
        ...
        if (fullContent.trim()) {
          ...
          if (!savedId) {
            ...
            toast.error("Erro ao salvar resposta. Por favor, recarregue a página.")
          }
        }
      } catch (error) {
  ...
  const loadHistory = useCallback(
    async (retryCount = 0) => {
      ...
      try {
        const response = await fetch(`/api/chat-history/onboarding?chatSessionId=${chatSessionId}`)
        ...
        setLoadError("Não foi possível carregar o histórico. Clique para tentar novamente.")
      } finally {
  ...
  useEffect(() => {
    if (!isLoadingHistory && messages.length === 0 && !initialGreetingSentRef.current && !loadError && !isStreaming) {
      initialGreetingSentRef.current = true
      setTimeout(() => {
        sendMessageToAI("Olá! Estou pronto para começar meu treinamento.")
      }, 500)
    }
  }, [isLoadingHistory, messages.length, loadError, isStreaming, sendMessageToAI])
    ...
    <div className="flex min-h-screen flex-col bg-background">
      <header className="sticky top-0 z-10 border-b bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/60">
        <div className="container flex h-14 md:h-16 items-center gap-2 md:gap-4 px-3 md:px-4">
          ...
          <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
            <div className="text-sm md:text-base font-semibold truncate">Farben</div>
            <div className="min-w-0 flex-1">
              {isEditingTitle ? (
              ...
              <p className="text-xs md:text-sm text-muted-foreground truncate">{roleTitle}</p>
            </div>
          </div>
        </div>
      </header>
            ...
            <div className="mb-3">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">Sugestões para começar</span>
                </div>
                {suggestionsExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronUp className="h-4 w-4" />}
              </button>
              {suggestionsExpanded && (
