...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: columns } = await supabase.from("kanban_columns").select("id").eq("board_id", id)
    ...
    // Delete the board
    const { error } = await supabase.from("kanban_boards").delete().eq("id", id).eq("user_id", user.id)
