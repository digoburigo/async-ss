...
function expandRecurringEvents(events: any[], rangeStart: Date, rangeEnd: Date): any[] {
  ...
  for (const event of events) {
    ...
    // Parse the rrule
    const rruleParts = event.rrule.split(";").reduce((acc: Record<string, string>, part: string) => {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (expandRecurring && startDate && endDate) {
      ...
      processedEvents = expandRecurringEvents(events || [], rangeStart, rangeEnd)
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, description, start_time, end_time, all_day, color, rrule } = body
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const body = await request.json()
    const { id, title, description, start_time, end_time, all_day, color, rrule } = body
