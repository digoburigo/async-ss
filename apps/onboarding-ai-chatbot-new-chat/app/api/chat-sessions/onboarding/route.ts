...
export async function GET(req: Request) {
  try {
    ...
    console.log("[v0] Loading chat sessions for user:", user.id, "type:", employeeType)
...
export async function POST(req: Request) {
  try {
    ...
    if (!employeeType) {
      return NextResponse.json({ error: "Missing employee type" }, { status: 400 })
    }
    ...
    console.log("[v0] Creating new chat session for user:", user.id, "type:", employeeType)
    ...
    if (chatUserError) {
      console.error("[v0] Error creating/updating chat user:", chatUserError.message)
...
export async function DELETE(req: Request) {
  try {
    ...
    const { error } = await supabase.from("onboarding_chat_sessions").delete().eq("id", chatSessionId)
