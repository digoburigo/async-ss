...
async function extractTextFromPDF(buffer: ArrayBuffer): Promise<string> {
  ...
  let extracted = textParts.join(" ").replace(/\\n/g, "\n").replace(/\\r/g, "").replace(/\s+/g, " ").trim()
  ...
  if (extracted.length < 100) {
    ...
    if (asciiText) {
      extracted = asciiText
        .filter((t) => !t.includes("stream") && !t.includes("endstream") && !t.includes("/Type"))
        .join(" ")
        .substring(0, 15000)
    }
  }
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (doc.file_url) {
      try {
        ...
        if (doc.mime_type === "application/pdf") {
          ...
          console.log("[v0] Extracted PDF text length:", documentContent.length)
        } else if (doc.mime_type?.includes("text") || doc.file_name?.endsWith(".txt")) {
    ...
    console.log("[v0] Has significant content:", hasContent, "Length:", documentContent?.length || 0)
...
Ao final, inclua a nota de aderência no formato: "NOTA_ADERENCIA: XX" onde XX é um número de 0 a 100.`
    ...
    const adherenceScore = scoreMatch ? Number.parseInt(scoreMatch[1], 10) : null
