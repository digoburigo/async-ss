...
export async function POST(request: Request) {
  try {
    ...
    const { question_id, candidate_id, position, department, question_type, current_question, is_ai_generated } = body

    if (!question_id || !candidate_id || !position || !question_type || !current_question) {
      return NextResponse.json({ error: "Missing required fields" }, { status: 400 })
    }

    // Generate a new question using AI with context from the current question
    const skillTypeLabel = question_type === "hard_skills" ? "Hard Skills (técnica)" : "Soft Skills (comportamental)"
