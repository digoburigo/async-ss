...
export async function POST(req: Request) {
  try {
    ...
    let { data: savedMessage, error } = await supabase.from("group_chat_messages").insert(messageData).select().single()
    ...
    if (error && error.message?.includes("session_id")) {
      ...
      const retry = await supabase.from("group_chat_messages").insert(messageData).select().single()
