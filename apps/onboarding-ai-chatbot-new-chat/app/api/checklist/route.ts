...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("[v0] Loading checklist for user:", user.id, "type:", employeeType)
    ...
    if (stepsError) {
      ...
      return NextResponse.json({ error: "Failed to fetch checklist steps" }, { status: 500 })
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[v0] Updating progress for user:", user.id, "step:", stepId, "completed:", completed)
    ...
    try {
      ...
      if (error) {
        if (error.code === "23502" && error.message?.includes("session_id")) {
          ...
          console.log("[v0] Progress updated successfully (backward compatibility mode)")
