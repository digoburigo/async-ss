...
export async function GET(request: Request) {
  try {
    ...
    let query = supabase.from("meetings").select("*").eq("user_id", user.id).order("meeting_date", { ascending: false })
...
export async function POST(request: Request) {
  try {
    ...
    if (create_calendar_event && meeting) {
      ...
      const endDate = new Date(meetingDate.getTime() + (meeting.duration_minutes || 60) * 60000)
      ...
      const { error: calendarError } = await supabase.from("calendar_events").insert(calendarEventData)
