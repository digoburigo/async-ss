...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await supabase.from("career_landing_pages").select("*").eq("id", id).single()
    ...
    const job_ids = jobAssociations?.map((assoc) => assoc.job_position_id) || []
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { title, slug, description, company_name, company_description, is_active, job_ids } = body
    ...
    if (job_ids !== undefined) {
      ...
      await supabase.from("career_landing_page_jobs").delete().eq("landing_page_id", id)
      ...
      if (job_ids.length > 0) {
        ...
        const { error: jobError } = await supabase.from("career_landing_page_jobs").insert(jobAssociations)
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { error } = await supabase.from("career_landing_pages").delete().eq("id", id)
