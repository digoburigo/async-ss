...
export async function GET(request: Request) {
  try {
    ...
    let query = supabase.from("offboarding_processes").select("*").order("created_at", { ascending: false })
...
export async function POST(request: Request) {
  try {
    ...
    const { user_id, employee_name, employee_email, department, last_working_day, reason, notes } = body
    ...
    const { data: steps } = await supabase.from("offboarding_checklist_steps").select("id")
    ...
    if (steps && steps.length > 0) {
      ...
      const { error: progressError } = await supabase.from("offboarding_progress").insert(progressEntries)
