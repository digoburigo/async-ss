...
export async function POST(request: Request) {
  try {
    ...
    const { process_id, task_title, task_description, assigned_to, assigned_to_name, priority, due_date } = body
...
export async function DELETE(request: Request) {
  try {
    ...
    const { error } = await supabase.from("offboarding_handover_tasks").delete().eq("id", taskId)
