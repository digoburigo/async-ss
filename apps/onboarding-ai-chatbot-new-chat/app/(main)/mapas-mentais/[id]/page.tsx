    ...
    setNodes((nds) =>
      nds.map((node) =>
        node.id === selectedNode.id
          ? {
              ...node,
              data: { ...node.data, label: editNodeLabel, color: editNodeColor },
            }
          : node,
      ),
    )
    ...
    <div className="h-[calc(100vh-64px)] flex flex-col">
      <div className="flex items-center justify-between p-4 border-b bg-background">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => router.push("/mapas-mentais")}>
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <div>
            <h1 className="text-xl font-bold">{mindmap.title}</h1>
            {mindmap.description && <p className="text-sm text-muted-foreground">{mindmap.description}</p>}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={handleAddNode} className="gap-2 bg-transparent">
      ...
      <div className="flex-1">
        ...
        >
          <Controls />
          <MiniMap nodeColor={(node) => node.data.color || "#3b82f6"} maskColor="rgba(0,0,0,0.1)" />
          <Background variant={BackgroundVariant.Dots} gap={20} size={1} />

          {selectedNode && (
            <Panel position="top-right" className="bg-background border rounded-lg p-3 shadow-lg">
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium truncate max-w-[150px]">{selectedNode.data.label}</span>
                ...
                {nodeColors.map((color) => (
                  <button
                    ...
                    className={cn(
                      ...
                      editNodeColor === color.value ? "border-primary ring-2 ring-primary/30" : "border-transparent",
                    )}
