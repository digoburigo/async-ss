...
export default function GestaoCurriculosPage() {
  ...
  const [resumeInsights, setResumeInsights] = useState<ResumeInsights | null>(
    null
  );
  ...
  const fetchAvailableJobs = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setAvailableJobs(
          data.filter((job: JobPosition) => job.status === "active")
        );
      }
    } catch (error) {
  ...
  const handleUpdateLandingPage = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/career/landing-pages/${selectedLandingPage.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ ...formData, job_ids: validJobIds }),
  ...
  const handleUpdateApplicationStatus = async (
    ...
    try {
      const response = await fetch(
        `/api/career/applications/${applicationId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const getAvailableJobsForSelect = (currentIndex: number) => {
    const selectedInOtherSelects = jobSelections.filter(
      (_, i) => i !== currentIndex
    );
    return availableJobs.filter(
      (job) => !selectedInOtherSelects.includes(job.id)
    );
  };
  ...
  const getApplicationsCount = (landingPageId: string) =>
    applications.filter((app) => app.landing_page_id === landingPageId).length;
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-muted-foreground">
              Gerencie suas landing pages de carreiras e candidaturas
            </p>
          </div>
      ...
      <div className="flex-1 overflow-auto p-6">
        <Tabs onValueChange={setActiveTab} value={activeTab}>
          <TabsList className="mb-6">
            <TabsTrigger className="gap-2" value="landing-pages">
              <Link2 className="h-4 w-4" />
              Landing Pages
            </TabsTrigger>
            <TabsTrigger className="gap-2" value="applications">
              ...
              <Card>
                <CardContent className="flex flex-col items-center justify-center py-12">
                  ...
                  <p className="mb-4 text-center text-muted-foreground">
                    Crie sua primeira landing page para começar a receber
                    candidaturas
                  </p>
                  <Button onClick={() => setIsCreateDialogOpen(true)}>
                    <Plus className="mr-2 h-4 w-4" />
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem
                              onClick={() =>
                                window.open(`/carreiras/${page.slug}`, "_blank")
                              }
                            >
          ...
          {/* Candidaturas Tab */}
          <TabsContent className="mt-6" value="applications">
            {applications.length === 0 ? (
              <Card>
                <CardContent className="py-12 text-center">
                  ...
                  <p className="text-muted-foreground">
                    As candidaturas aparecerão aqui quando candidatos se
                    inscreverem através das landing pages.
                  </p>
                </CardContent>
              </Card>
            ) : (
                  ...
                  <TableBody>
                    {applications.map((application) => (
                      <TableRow key={application.id}>
                        ...
                        <TableCell>
                          {application.preboarding_job_positions?.title ||
                            "Não especificada"}
                        </TableCell>
                        <TableCell>
                          {application.career_landing_pages?.title || "-"}
                        </TableCell>
                        <TableCell>
                          {format(
      ...
      >
        <DialogContent className="flex max-h-[90vh] max-w-2xl flex-col p-0">
          <DialogHeader className="border-b px-6 pt-6 pb-4">
            <DialogTitle>
              {isEditDialogOpen ? "Editar Landing Page" : "Nova Landing Page"}
            </DialogTitle>
            <DialogDescription>
              Configure as informações da sua página de carreiras
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid gap-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-muted-foreground text-xs">
                    {jobSelections.filter((id) => id !== "").length} vaga(s)
                    adicionada(s)
                  </span>
                </div>
                ...
                {availableJobs.length === 0 ? (
                  <div className="rounded-lg border p-4 text-center">
                    <p className="text-muted-foreground text-sm">
                      Nenhuma vaga cadastrada. Crie vagas em &quot;Gestão de
                      Cargos&quot; primeiro.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {jobSelections.map((selectedJobId, index) => (
                      <div className="flex items-center gap-2" key={index}>
                        <Select
                          onValueChange={(value) =>
                            updateJobSelection(index, value)
                          }
                          value={selectedJobId}
                        >
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-muted-foreground text-sm">
                    Páginas inativas não são acessíveis publicamente
                  </p>
                </div>
                <Switch
      ...
      <Dialog onOpenChange={setIsDeleteDialogOpen} open={isDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Tem certeza que deseja excluir &quot;{selectedLandingPage?.title}
              &quot;? Esta ação não pode ser desfeita e todas as candidaturas
          ...
          <DialogFooter>
            ...
            <Button onClick={handleDeleteLandingPage} variant="destructive">
              Excluir
            </Button>
          </DialogFooter>
        </DialogContent>
            ...
            {selectedApplication && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div className="min-w-0 space-y-1">
                    ...
                    <p className="break-words font-medium">
                      {selectedApplication.preboarding_job_positions?.title ||
                        "Não especificada"}
                    </p>
                  </div>
                  <div className="space-y-1">
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                      {selectedApplication.linkedin_url && (
                        <Button
                          ...
                          variant="outline"
                        >
                          <Linkedin className="mr-2 h-4 w-4 flex-shrink-0 text-[#0A66C2]" />
                      ...
                      {selectedApplication.instagram_url && (
                        <Button
                          ...
                          variant="outline"
                        >
                          <Instagram className="mr-2 h-4 w-4 flex-shrink-0 text-[#E4405F]" />
                      ...
                      {selectedApplication.facebook_url && (
                        <Button
                          ...
                          variant="outline"
                        >
                          <Facebook className="mr-2 h-4 w-4 flex-shrink-0 text-[#1877F2]" />
                      ...
                      {selectedApplication.twitter_url && (
                        <Button
                          ...
                          variant="outline"
                        >
                          <Twitter className="mr-2 h-4 w-4 flex-shrink-0 text-[#1DA1F2]" />
                      ...
                      {selectedApplication.portfolio_url && (
                        <Button
                          ...
                          variant="outline"
                        >
                          <Globe className="mr-2 h-4 w-4 flex-shrink-0 text-purple-600" />
                ...
                {selectedApplication.cover_letter && (
                  <div className="space-y-2">
                    <p className="font-medium text-sm">Carta de Apresentação</p>
                    <p className="whitespace-pre-wrap break-words rounded-md bg-muted p-3 text-muted-foreground text-sm">
                      {selectedApplication.cover_letter}
                    </p>
                  </div>
                )}
                ...
                {selectedApplication.resume_url && (
                  <div className="space-y-3">
                    <p className="font-medium text-sm">Currículo</p>
                      ...
                      <div className="mt-4 space-y-4 rounded-lg border bg-muted/50 p-4">
                        {isLoadingInsights ? (
                          ...
                          <>
                            ...
                            <div>
                              <h4 className="mb-2 font-semibold text-orange-600 text-sm">
                                ⚠ Pontos de Melhoria
                              </h4>
                              <ul className="list-inside list-disc space-y-1">
                                {resumeInsights.weaknesses.map(
                                  (weakness, idx) => (
                                    <li
                                      className="break-words text-muted-foreground text-sm"
                                      key={idx}
