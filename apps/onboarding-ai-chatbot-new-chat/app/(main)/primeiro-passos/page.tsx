...
import { CheckCircle2, Circle, Loader2, AlertCircle, Trophy, User, RefreshCw, Sparkles } from "lucide-react"
  ...
  stock_manager: [
    {
      ...
      options: ["Abaixo de 0°C", "Entre 10°C e 30°C", "Acima de 40°C", "Não importa a temperatura"],
      correctAnswer: 1,
      explanation:
    ...
    },
    {
      ...
      options: ["LIFO (Last In, First Out)", "FIFO (First In, First Out)", "Aleatório", "Por cor"],
      correctAnswer: 1,
      explanation:
    ...
    {
      id: "q3",
      question: "Qual é o procedimento correto ao receber uma nova remessa de produtos?",
      options: [
...
export default function PrimeirosPassosPage() {
  ...
  const loadChecklist = useCallback(
    async (retryCount = 0) => {
      ...
      try {
        const response = await fetch(`/api/checklist?employeeType=${selectedType}`)
        const data = await response.json()

        if (!response.ok) {
          throw new Error(data.details || data.error || "Erro ao carregar checklist")
        }
        ...
        const errorMessage = error instanceof Error ? error.message : "Erro ao carregar checklist"

        // Retry up to 2 times on network errors
        if (retryCount < 2 && errorMessage.includes("fetch")) {
          setTimeout(() => loadChecklist(retryCount + 1), 1000 * (retryCount + 1))
          return
        }
  ...
  const toggleStep = async (stepId: string, currentCompleted: boolean) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.details || data.error || "Erro ao atualizar progresso")
      }
  ...
  const quizPercentage = questions.length > 0 ? Math.round((quizScore / questions.length) * 100) : 0
        ...
        {error ? (
          ...
          <Card className="max-w-md p-8 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Você precisa estar autenticado para acessar os Primeiros Passos.</p>
            </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted p-4 md:p-8">
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="text-center space-y-4">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-balance">Primeiros Passos</h1>
            <p className="text-lg text-muted-foreground text-balance max-w-2xl mx-auto">
              Selecione seu tipo de função para começar sua jornada de integração personalizada.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground leading-relaxed">
                    Aprenda sobre nossos produtos, técnicas de venda, atendimento ao cliente e como ajudar nossos
                    clientes a escolher as melhores tintas.
                  </p>
                </div>
            ...
            >
              <div className="space-y-4">
                ...
                <div>
                  <h3 className="text-2xl font-bold mb-2">Gerente de Estoque</h3>
                  <p className="text-muted-foreground leading-relaxed">
                    Domine nosso sistema de inventário, processos de recebimento, organização de produtos e controle de
                    qualidade.
                  </p>
                </div>
                <Button className="w-full group-hover:bg-primary/90" size="lg">
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            {currentQuestionIndex < questions.length - 1 ? (
              <Button
                ...
                disabled={selectedAnswers[currentQuestion.id] === undefined || isPending}
              >
                Próxima →
              </Button>
            ) : (
              <Button onClick={handleSubmitQuiz} disabled={!allQuestionsAnswered} className="min-w-[120px]">
                Finalizar Quiz
              </Button>
            )}
          </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-amber-800 dark:text-amber-200 mt-1">
                    Por favor, responda todas as questões antes de finalizar o quiz.
                  </p>
                </div>
              </div>
            </Card>
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted p-4 md:p-8">
        <div className="max-w-3xl mx-auto space-y-8">
          ...
          <Card
            className={cn(
              "p-8",
              passed ? "bg-green-500/10 border-green-500/50" : "bg-amber-500/10 border-amber-500/50",
            )}
          >
            <div className="text-center space-y-4">
              <div
              ...
              <div>
                <h2 className="text-3xl font-bold mb-2">{passed ? "Parabéns!" : "Quase lá!"}</h2>
                <p className="text-lg text-muted-foreground">
                  Você acertou {quizScore} de {questions.length} questões ({quizPercentage}%)
                </p>
              </div>
              {passed ? (
                <p className="text-muted-foreground max-w-md mx-auto">
                  Excelente trabalho! Você demonstrou um ótimo entendimento dos primeiros passos na Farben.
                </p>
              ) : (
                <p className="text-muted-foreground max-w-md mx-auto">
                  Revise os itens marcados abaixo e tente novamente. Você precisa de pelo menos 70% para passar.
                </p>
              )}
            </div>
          </Card>
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            {!passed && <Button onClick={handleRetakeQuiz}>Tentar Novamente</Button>}
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted p-4 md:p-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h1 className="text-3xl md:text-4xl font-bold">Sua Lista de Integração</h1>
        ...
        {error && (
          <Alert variant="destructive">
            ...
            <AlertDescription className="flex items-center justify-between">
              <span>{error}</span>
              <Button variant="outline" size="sm" onClick={() => loadChecklist(0)} className="ml-4">
            ...
            {steps.map((step, index) => {
              ...
              return (
                <Card
                  ...
                  onClick={() => !isToggling && toggleStep(step.id, step.completed)}
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 space-y-2">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <Checkbox
                          ...
                          disabled={isToggling}
                          onCheckedChange={() => !isToggling && toggleStep(step.id, step.completed)}
                        />
                      </div>
                      <p className={cn("text-sm", step.completed ? "text-muted-foreground" : "text-foreground")}>
                        {step.description}
                      </p>
        ...
        {progress === 100 && (
          <Card className="p-8 bg-primary/5 border-primary">
            <div className="text-center space-y-4">
              ...
              <p className="text-muted-foreground max-w-md mx-auto">
                Você completou todos os passos da sua integração. Agora teste seus conhecimentos com um quiz rápido!
              </p>
