...
import { Users, Plus, Search, MoreHorizontal, Mail, Briefcase, Star, Trash2, Eye, Filter } from "lucide-react"
  ...
  const handleAddCandidate = async () => {
    ...
    try {
      const res = await fetch("/api/preboarding/candidates", {
        ...
        body: JSON.stringify({
          ...formData,
          expected_salary: formData.expected_salary ? Number.parseFloat(formData.expected_salary) : null,
    ...
    return (
      <div className="flex items-center gap-0.5">
        {[1, 2, 3, 4, 5].map((star) => (
          <Star
            key={star}
            className={cn("h-3 w-3", star <= rating ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground")}
          />
        ))}
      </div>
    )
    ...
    <div className="flex-1 p-6 overflow-hidden flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Gerencie seus candidatos em processo seletivo</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-sm text-muted-foreground">
              {jobFilter === "all" ? "Total de Candidatos" : "Candidatos na Vaga"}
            </div>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold">
              {stages.length > 0 ? getCandidatesByStage(stages[stages.length - 1]?.id).length : 0}
            </div>
                        ...
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <Avatar className="h-8 w-8">
                              <AvatarFallback className="text-xs" style={{ backgroundColor: `${stage.color}40` }}>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => router.push(`/preboarding/${candidate.id}`)}>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b bg-muted/50">
                    ...
                    <th className="text-left p-4 font-medium">Salário Esperado</th>
                ...
                <tbody>
                  {filteredCandidates.map((candidate) => (
                    <tr key={candidate.id} className="border-b hover:bg-muted/50">
                      ...
                      <td className="p-4">
                        {getJobTitle(candidate.job_position_id) ? (
                          <Badge variant="secondary">{getJobTitle(candidate.job_position_id)}</Badge>
                        ) : (
                      ...
                      <td className="p-4">
                        {candidate.stage && (
                          <Badge
                            style={{ backgroundColor: `${candidate.stage.color}20`, color: candidate.stage.color }}
                          >
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Preencha as informações do novo candidato</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <Select value={formData.job_position_id} onValueChange={handleJobPositionChange}>
                  <SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                {formData.job_position_id && (
                  <p className="text-xs text-muted-foreground mt-1">Preenchido automaticamente pela vaga</p>
                )}
              </div>
              <div>
                ...
                <Input
                  ...
                  className={formData.job_position_id && formData.department ? "bg-muted" : ""}
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
