...
import { WEEK_CELLS_HEIGHT } from "./constants";
...
interface DayViewProps {
  ...
  onEventSelect: (event: EventCalendarType) => void;
...
export function DayView({
  ...
  const positionedEvents = useMemo(() => {
    ...
    const columns: { event: EventCalendarType; end: Date }[][] = [];
    ...
    for (const event of sortedEvents) {
      ...
      while (!placed) {
        if (columns[columnIndex]) {
          ...
          const overlaps = columns.at(columnIndex)?.some((col) =>
            areIntervalsOverlapping(
      ...
      columns.at(columnIndex)?.push({ event, end: adjustedEnd });
  ...
  const handleEventClick = (event: EventCalendarType, e: React.MouseEvent) => {
    e.stopPropagation();
    onEventSelect(event);
  };

  const showAllDaySection = (allDayEvents?.length ?? 0) > 0;
  ...
  return (
    <div className="flex h-full flex-col overflow-auto">
      {showAllDaySection ? (
        <div className="sticky top-0 z-20 border-border/70 border-t bg-muted/50">
          <div className="grid grid-cols-[3rem_1fr] sm:grid-cols-[4rem_1fr]">
            <div className="relative">
              <span className="absolute bottom-0 left-0 h-6 w-16 max-w-full pe-1 text-right text-[10px] text-muted-foreground/70 sm:pe-2 sm:text-xs">
                Dia todo
              </span>
            </div>
      ...
      <div className="grid grid-cols-[3rem_1fr] border-border/70 border-t sm:grid-cols-[4rem_1fr]">
        <div>
