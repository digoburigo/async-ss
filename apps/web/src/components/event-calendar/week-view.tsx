...
export function WeekView({
	...
	const processedDayEvents = useMemo(() => {
		const result = days.map((day) => {
			...
			const columns: { event: EventCalendar; end: Date }[][] = [];
	...
	return (
		<div className="flex h-full flex-col">
			<div className="border-border/70 bg-background/80 sticky top-16 z-30 grid grid-cols-8 border-b backdrop-blur-md">
				...
				<div className="border-border/70 bg-muted/50 border-b">
					<div className="grid grid-cols-8">
						<div className="border-border/70 relative border-r">
							<span className="text-muted-foreground/70 absolute bottom-0 left-0 h-6 w-16 max-w-full pe-1 text-right text-[10px] sm:pe-2 sm:text-xs">
								Dia todo
							</span>
						</div>
			...
			<div className="grid flex-1 grid-cols-8">
				...
				{days.map((day, dayIndex) => (
					...
					>
						{/* Positioned events */}
						{processedDayEvents.at(dayIndex)?.map((positionedEvent) => (
							<div
