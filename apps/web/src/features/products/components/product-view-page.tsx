...
export default function ProductViewPage({ productId }: TProductViewPageProps) {
  ...
  const { data: product, isFetching } = client.product.useFindUnique(
    ...
    {
      enabled: !!activeOrganization?.id && !!productId,
    }
  );
  ...
  const handleSubmit = async (data: {
    ...
    updateProduct({
      data: {
        ...
        unit: data.unit || null,
        costPrice: data.costPrice ? Number.parseFloat(data.costPrice) : null,
        salePrice: data.salePrice ? Number.parseFloat(data.salePrice) : null,
        minimumStock: data.minimumStock
          ? Number.parseInt(data.minimumStock, 10)
          : null,
    ...
    return (
      <div className="flex flex-1 items-center justify-center">
        <div className="text-center">
          <h2 className="font-bold text-2xl">Produto não encontrado</h2>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h1 className="font-bold text-3xl tracking-tight">Editar Produto</h1>
